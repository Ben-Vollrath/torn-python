...
class UserPropertyDetailsExtendedForSale(BaseModel):
    """
    ...
    status: StrictStr
    cost: StrictInt
    __properties: ClassVar[List[str]] = ["id", "owner", "property", "happy", "upkeep", "market_price", "modifications", "staff", "used_by", "status", "cost"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['for_sale']):
            raise ValueError("must be one of enum values ('for_sale')")
        return value
