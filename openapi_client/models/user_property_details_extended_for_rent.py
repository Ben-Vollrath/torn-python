...
class UserPropertyDetailsExtendedForRent(BaseModel):
    """
    ...
    status: StrictStr
    cost: StrictInt
    cost_per_day: StrictInt
    rental_period: StrictInt
    renter_asked: Optional[BasicUser] = None
    __properties: ClassVar[List[str]] = ["id", "owner", "property", "happy", "upkeep", "market_price", "modifications", "staff", "used_by", "status", "cost", "cost_per_day", "rental_period", "renter_asked"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['for_rent']):
            raise ValueError("must be one of enum values ('for_rent')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of renter_asked
        if self.renter_asked:
            _dict['renter_asked'] = self.renter_asked.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "cost": obj.get("cost"),
            "cost_per_day": obj.get("cost_per_day"),
            "rental_period": obj.get("rental_period"),
            "renter_asked": BasicUser.from_dict(obj["renter_asked"]) if obj.get("renter_asked") is not None else None
        })
        return _obj
